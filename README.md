...
OAuth providers (Wahoo, Withings, Polar, FatSecret) also need `OAUTH_REDIRECT_URI` set to your deployment's callback URL (e.g. `https://your-domain.example.com/callback`). After adding credentials, click the provider tile on the Data Sources page to complete the OAuth flow.
