...
describe("parseDuringRange", () => {
  it("parses a standard inclusive-exclusive range", () => {
    const result = parseDuringRange(
      "['2026-03-12T21:37:00.000Z','2026-03-12T21:56:00.000Z')",
    );
  ...
  it("parses a fully inclusive range", () => {
    const result = parseDuringRange(
      "['2024-01-15T08:00:00.000Z','2024-01-15T09:30:00.000Z']",
    );
  ...
  it("throws on invalid range string", () => {
    expect(() => parseDuringRange("invalid")).toThrow(
      "Could not parse 'during' range: invalid",
    );
  });
