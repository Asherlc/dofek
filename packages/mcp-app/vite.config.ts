...
function inlineMcpAppBundle(): Plugin {
  return {
    ...
    async closeBundle() {
      ...
      const scriptSource = html.match(/<script type="module" crossorigin src="(\/assets\/[A-Za-z0-9._/-]+)"><\/script>/)?.[1];
      if (!scriptSource || scriptSource.includes("..")) {
        throw new Error("MCP App bundle did not produce a safe module script path.");
      }
