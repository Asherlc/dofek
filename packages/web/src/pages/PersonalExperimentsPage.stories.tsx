...
import { PersonalExperimentsPage } from "./PersonalExperimentsPage.tsx";
...
function createMockObservable(
  ...
  const result: OperationResultObservable<AppRouter, unknown> = {
    subscribe(observer) {
      ...
      } else if (path === "personalExperiments.list") {
        observer.next?.({
          result: { data: scenario === "active" ? [activeExperiment] : [] },
        });
      } else if (path === "personalExperiments.analysis") {
  ...
  return (
    <trpc.Provider client={trpcClient} queryClient={queryClient}>
      <QueryClientProvider client={queryClient}>
        <PersonalExperimentsPage search={{ outcomeMetricId: "hrv", lagDays: 1 }} />
      </QueryClientProvider>
    </trpc.Provider>
  );
}
...
const meta = {
  ...
  component: PersonalExperimentsPage,
  parameters: {
    layout: "fullscreen",
  },
} satisfies Meta<typeof PersonalExperimentsPage>;
