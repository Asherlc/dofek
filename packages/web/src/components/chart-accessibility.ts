...
export function buildChartSummary(option: Record<string, unknown>): string {
  ...
  if (uniqueSeriesNames.length > 0) {
    return `Chart showing ${formatList(uniqueSeriesNames)}. Use the chart data table for exact values.`;
  }
  ...
  if (uniqueAxisNames.length > 0) {
    return `Chart comparing ${formatList(uniqueAxisNames)}. Use the chart data table for exact values.`;
  }
  ...
  if (firstSeriesType !== undefined) {
    return `${formatChartType(firstSeriesType)} chart. Use the chart data table for exact values.`;
  }

  return "Chart. Use the chart data table for exact values.";
}
