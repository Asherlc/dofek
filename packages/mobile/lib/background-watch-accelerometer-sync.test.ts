...
vi.mock("./accelerometer-sync", () => ({
  syncAccelerometerToServer: vi.fn(() => Promise.resolve({ inserted: 0, recording: true })),
}));
...
describe("backgroundWatchAccelerometerSync", () => {
  ...
  it("initializes without throwing when Watch is paired", async () => {
    ...
    await expect(initBackgroundWatchAccelerometerSync(mockClient)).resolves.not.toThrow();
  });
