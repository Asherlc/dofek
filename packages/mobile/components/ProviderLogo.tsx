...
export function ProviderLogo({ provider, size = 20 }: ProviderLogoProps) {
  const logoType = providerLogoType(provider);

  if (logoType === "png") {
    const source = PNG_LOGOS[provider];
    if (source) {
      return (
        <Image
          source={source}
  ...
  if (logoType === "svg") {
    const svgString = SVG_LOGOS[provider];
    if (svgString) {
      return <SvgXml xml={svgString} width={size} height={size} />;
    }
  }

  // Styled letter fallback
