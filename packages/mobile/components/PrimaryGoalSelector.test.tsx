...
const { PrimaryGoalSelector } = await import("./PrimaryGoalSelector");
...
describe("PrimaryGoalSelector", () => {
  ...
  it("marks the currently selected goal", () => {
    mocks.query.data = { key: "primaryGoal", value: "sleepConsistency" };
    render(<PrimaryGoalSelector />);

    expect(
      screen.getByRole("button", { name: /Sleep consistency/ }).getAttribute("aria-selected"),
    ).toBe("true");
  });
  ...
  it("keeps the selected goal visible and reports a background read error", async () => {
