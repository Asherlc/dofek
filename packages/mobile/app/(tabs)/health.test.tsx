...
vi.mock("react-native", () => ({
  View: ({ children, ...props }: { children?: React.ReactNode } & Record<string, unknown>) =>
    React.createElement("div", stripStyles(props), children),
  Text: ({ children, ...props }: { children?: React.ReactNode } & Record<string, unknown>) =>
    React.createElement("span", stripStyles(props), children),
  ...
  TouchableOpacity: ({
    ...
    disabled,
    ...props
  }: { children?: React.ReactNode; onPress?: () => void; disabled?: boolean } & Record<
    string,
    unknown
  >) =>
    React.createElement(
      "button",
      { type: "button", onClick: onPress, disabled, ...stripStyles(props) },
      children,
    ),
  StyleSheet: {
...
describe("HealthScreen", () => {
  ...
  it("uses selected range when syncing", async () => {
    ...
    const firstCallStartDate = String(mockQueryDailyStatistics.mock.calls[0][1]);
  ...
  it("queries skin temperature from HealthKit during sync", async () => {
    ...
    const queriedTypes = mockQueryQuantitySamples.mock.calls.map((call: unknown[]) =>
      String(call[0]),
    );
    expect(queriedTypes).toContain("HKQuantityTypeIdentifierAppleSleepingWristTemperature");
  });
  ...
  it("normalizes missing workout optional fields to null before sync", async () => {
    ...
    } = mockPushWorkoutsMutate.mock.calls[0][0];
