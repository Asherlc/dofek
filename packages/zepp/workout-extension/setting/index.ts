...
AppSettingsPage({
  ...
  build(props: {
    ...
    if (imuStatus.requiresAccountBinding === true && connectionState === "connected") {
      blocks.push(
        Button({
          label: "Assign retained motion recordings",
          color: "primary",
          style: { marginTop: "1em" },
          onClick: () => {
            toggle(props.settingsStorage, STORAGE_KEYS.CMD_ASSIGN_LEGACY_IMU);
          },
        }),
      );
    }
