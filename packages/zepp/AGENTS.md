...
- **Shared Settings UI**: Keep the normal app and Workout Extension Settings
  pages as thin registrations of `src/settings-page.ts`. Zepp supplies Settings
  components as runtime bindings and reruns `build` after Settings Storage
  changes; call those components directly and keep shared state derived from
  storage during each build ([Settings App registration](https://docs.zepp.com/docs/guides/framework/app-settings/register/)).
