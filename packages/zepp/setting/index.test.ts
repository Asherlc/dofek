...
describe("normal Zepp app settings", () => {
  ...
  it("requires an explicit account choice before recovering retained motion", () => {
    ...
    button("Assign retained motion recordings").onClick();

    expect(settingsStorage.setItem).toHaveBeenCalledWith(STORAGE_KEYS.CMD_ASSIGN_LEGACY_IMU, "1");
  });
