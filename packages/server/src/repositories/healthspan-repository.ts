...
export class HealthspanRepository {
  ...
  async getScore(weeks: number, endDate: string): Promise<HealthspanResult> {
    ...
    const rows = await executeWithSchema(
      ...
      sql`WITH sleep_data AS (
            SELECT
              ...
              duration_minutes,
              EXTRACT(HOUR FROM started_at AT TIME ZONE ${this.#timezone}) * 60 + EXTRACT(MINUTE FROM started_at AT TIME ZONE ${this.#timezone}) AS bedtime_minutes
          ...
          metrics_agg AS (
            SELECT
              ...
              (SELECT vo2max FROM fitness.v_daily_metrics
               WHERE user_id = ${this.#userId} AND vo2max IS NOT NULL
          ...
          hr_zone_time AS (
            ...
            JOIN LATERAL (
              ...
              WHERE dm2.user_id = asum.user_id
                AND dm2.date <= (asum.started_at AT TIME ZONE ${this.#timezone})::date
  ...
  #buildResult(row: z.infer<typeof rawRowSchema>): HealthspanResult {
    ...
    const metricsWithData = metricDefs.filter((metric) => metric.value != null);
    ...
    const history = weeklyHistory.map((week) => {
      ...
      return { weekStart: week.week_start, score: weekScore };
    });
