...
describe("BodyRepository", () => {
  ...
  it("computes rolling weight from direct daily observations and uses a 27-day lookback", async () => {
    const { repo, query } = makeRepository([
      {
        date: "2026-05-01",
        recorded_at: "2026-05-01T15:00:00Z",
        provider_id: "withings",
        body_priority: 10,
        weight_kg: 80,
        body_fat_pct: null,
      ...
      {
        date: "2026-05-07",
        recorded_at: "2026-05-07T15:00:00Z",
        provider_id: "withings",
        body_priority: 10,
        weight_kg: 82,
        body_fat_pct: null,
      ...
      {
        date: "2026-05-08",
        recorded_at: "2026-05-08T15:00:00Z",
        provider_id: "withings",
        body_priority: 10,
        weight_kg: 84,
        body_fat_pct: null,
    ...
    const result = await repo.listReconciledRange("2026-05-08", "2026-05-08");

    expect(result).toHaveLength(1);
    expect(result[0]?.weightRolling).toEqual({
      average7dKg: 83,
      average28dKg: 82,
      observedDays7d: 2,
      observedDays28d: 3,
    });
    expect(query.mock.calls[0]?.[1]).toContain("addDays(toDate({startDate:String}), -27)");
  });

  it("does not select or average a non-positive weight but retains it in source provenance", async () => {
    const { repo } = makeRepository([
      {
        date: "2026-05-08",
        recorded_at: "2026-05-08T15:00:00Z",
        provider_id: "invalid_scale",
        body_priority: 1,
        weight_kg: 0,
        body_fat_pct: null,
        bmi: null,
    ...
    expect(result[0]).toMatchObject({
      ...
      sourceProviderByMetric: { weightKg: "valid_scale" },
      sources: [
        expect.objectContaining({ sourceProvider: "invalid_scale", weightKg: 0 }),
        expect.objectContaining({ sourceProvider: "valid_scale", weightKg: 80 }),
      ],
    });
  });
});
