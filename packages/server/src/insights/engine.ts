...
function joinByDate(
  ...
  for (let i = 0; i < joined.length; i++) {
    ...
    if (weights.length >= 5) {
      joined[i].weight_30d_avg = weights.reduce((a, b) => a + b, 0) / weights.length;
    }
    if (fats.length >= 5) {
      joined[i].body_fat_30d_avg = fats.reduce((a, b) => a + b, 0) / fats.length;
    }
    ...
    if (i >= BODY_WINDOW * 2 - 1) {
      ...
      if (weights.length >= 5 && prevWeights.length >= 5) {
        ...
        joined[i].weight_30d_delta = curAvg - prevAvg;
      }
      if (fats.length >= 5 && prevFats.length >= 5) {
        ...
        joined[i].body_fat_30d_delta = curAvg - prevAvg;
      }
    }
  }
...
export function computeInsights(
  ...
  if (conditionalCandidates.length > 0) {
    ...
    for (let i = 0; i < conditionalCandidates.length; i++) {
      if (significant[i]) {
        const { rawPValue: _, ...insight } = conditionalCandidates[i];
