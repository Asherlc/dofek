      ...
      const [sleepResults, bodyResults] = await Promise.all([
        executeWithSchema(
          ...
          WITH before_sleep AS (
            ...
            WHERE user_id = ${ctx.userId}
              AND started_at::date BETWEEN (${startDate}::date - ${windowDays}::int) AND (${startDate}::date - 1)
              AND NOT is_nap
          ),
          after_sleep AS (
            SELECT 'after' as period, *
            ...
            WHERE user_id = ${ctx.userId}
              AND ${
                endDate
                  ...
                  : sql`started_at::date BETWEEN ${startDate}::date AND (${startDate}::date + ${windowDays}::int)`
              }
              AND NOT is_nap
          ),
          combined AS (
        ...
        executeWithSchema(
          ...
          WITH before_body AS (
            ...
            WHERE user_id = ${ctx.userId}
              AND recorded_at::date BETWEEN (${startDate}::date - ${windowDays}::int) AND (${startDate}::date - 1)
          ),
          after_body AS (
            SELECT 'after' as period, *
            FROM fitness.v_body_measurement
            WHERE user_id = ${ctx.userId}
              AND ${
                endDate
                  ...
                  : sql`recorded_at::date BETWEEN ${startDate}::date AND (${startDate}::date + ${windowDays}::int)`
              }
          ),
          combined AS (
