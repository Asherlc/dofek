  ...
  overview: adminProcedure.query(async ({ ctx }) => {
    const rows = await executeWithSchema(
      ...
      sql`SELECT table_name, row_count FROM (
        ...
        UNION ALL SELECT 'accelerometer_sample', COUNT(*)::text FROM fitness.accelerometer_sample
    ...
    .query(async ({ ctx, input }) => {
      const [accounts, providers, sessions] = await Promise.all([
        ...
        executeWithSchema(
          ...
          sql`SELECT id, name, created_at::text
              FROM fitness.provider WHERE user_id = ${input.userId}
              ORDER BY created_at`,
        ),
        executeWithSchema(
          ctx.db,
