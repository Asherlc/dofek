-- 0069_subjective_inputs was ordered before the latest applied migration in some databases.
-- Keep this repair idempotent so it can run after either the skipped or the complete history.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conrelid = 'fitness.activity'::regclass
      AND conname = 'activity_perceived_exertion_range'
  ) THEN
...
CREATE TABLE IF NOT EXISTS fitness.body_region (
  id text PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS body_region_parent_sort_idx
...
CREATE TABLE IF NOT EXISTS fitness.subjective_check_in (
...
CREATE INDEX IF NOT EXISTS subjective_check_in_user_date_idx
ON fitness.subjective_check_in (user_id, date DESC);

CREATE TABLE IF NOT EXISTS fitness.subjective_symptom (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS subjective_symptom_region_idx
ON fitness.subjective_symptom (body_region_id);

CREATE TABLE IF NOT EXISTS fitness.injury_event (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES fitness.user_profile (id) ON DELETE CASCADE,
  kind text NOT NULL,
  body_region_id text NOT NULL REFERENCES fitness.body_region (id) ON DELETE RESTRICT,
...
CREATE INDEX IF NOT EXISTS injury_event_user_onset_idx
ON fitness.injury_event (user_id, onset_date DESC);

CREATE INDEX IF NOT EXISTS injury_event_body_region_idx
ON fitness.injury_event (body_region_id);

-- Keep the deployed view aligned with drizzle/_views/01_v_activity.sql.
