-- Preserve per-member source attribution for deduped activity groups.
...
absent_source_links AS (
  SELECT
    ...
    JSONB_AGG(
      ...
      ORDER BY t.provider_id ASC, t.id ASC
    ...
    (
      ...
      FROM (
        SELECT
          raw_entry.raw_key,
          raw_entry.raw_value,
          ROW_NUMBER() OVER (PARTITION BY raw_entry.raw_key ORDER BY r.prio ASC) AS raw_priority
