...
SET
  provider_type = activity_type::text,
  canonical_type = CASE
  ...
  modality = CASE activity_type::text
...
BEGIN
  IF EXISTS (
    ...
    WHERE canonical_type IS NULL OR provider_type IS NULL
  ) THEN
    RAISE EXCEPTION 'Canonical activity type backfill left unmapped rows';
  END IF;
END
$$;
