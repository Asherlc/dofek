-- Cross-type activity deduplication
--
-- Previously, v_activity required activity_type to match for two activities
-- to be considered duplicates. This meant the same workout recorded by
-- different providers with different type names (e.g., Wahoo "cycling" vs
-- RideWithGPS "other", or WHOOP "commuting" vs Apple Health "other") was
-- counted twice in training volume.
--
-- Fix: Remove the activity_type constraint from the pairs CTE. The 80%
-- time-overlap threshold is sufficient to identify duplicate activities
-- regardless of how each provider labels the type. The winning row's
-- activity_type (from the highest-priority provider) is used.
