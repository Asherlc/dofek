-- Fix activity_summary to join through v_activity (dedup view) instead of
-- the raw activity table. Without this, workouts synced from multiple
-- providers (e.g. Wahoo + Apple Health) appear as separate rows, causing
-- double-counted load in ramp rate, PMC, and other aggregate queries.

DROP MATERIALIZED VIEW fitness.activity_summary;
