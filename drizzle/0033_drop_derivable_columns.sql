-- ============================================================
-- Migration 0033: Drop derivable columns
-- ============================================================
-- Remove columns that store values derivable from other raw data:
-- - metric_stream.distance: computable from GPS (lat/lng) via haversine
...
-- Drop summary columns from activity_interval (all derivable from metric_stream)
ALTER TABLE fitness.activity_interval DROP COLUMN IF EXISTS avg_heart_rate;
ALTER TABLE fitness.activity_interval DROP COLUMN IF EXISTS max_heart_rate;
ALTER TABLE fitness.activity_interval DROP COLUMN IF EXISTS avg_power;
ALTER TABLE fitness.activity_interval DROP COLUMN IF EXISTS max_power;
ALTER TABLE fitness.activity_interval DROP COLUMN IF EXISTS avg_speed;
ALTER TABLE fitness.activity_interval DROP COLUMN IF EXISTS max_speed;
ALTER TABLE fitness.activity_interval DROP COLUMN IF EXISTS avg_cadence;
ALTER TABLE fitness.activity_interval DROP COLUMN IF EXISTS distance_meters;
ALTER TABLE fitness.activity_interval DROP COLUMN IF EXISTS elevation_gain;
