ALTER TABLE fitness.activity_interval
  ...
  ADD COLUMN target_zone integer,
...
ALTER TABLE fitness.activity_interval
  ADD CONSTRAINT activity_interval_source_provider_provider_id_fk
    FOREIGN KEY (source_provider) REFERENCES fitness.provider(id);
--> statement-breakpoint
ALTER TABLE fitness.activity_interval
  ADD CONSTRAINT activity_interval_source_activity_id_activity_id_fk
    FOREIGN KEY (source_activity_id) REFERENCES fitness.activity(id) ON DELETE SET NULL;
--> statement-breakpoint
ALTER TABLE fitness.activity_interval
  ADD CONSTRAINT activity_interval_source_kind
    CHECK (source_kind IS NULL OR source_kind IN ('provider_recorded', 'inferred'));
--> statement-breakpoint
ALTER TABLE fitness.activity_interval
...
ALTER TABLE fitness.activity_interval
  ADD CONSTRAINT activity_interval_work_recovery_kind
    CHECK (work_recovery_kind IS NULL OR work_recovery_kind IN ('work', 'recovery'));
