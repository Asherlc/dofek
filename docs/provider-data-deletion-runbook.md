...
7. The worker uses keyset pagination over `(generation, id)` in batches of 1,000 active IDs, selects each key's latest projected version before excluding existing tombstones, inserts exact-ID tombstones, and persists a checkpoint after every batch. The progress message reports both examined and deleted rows so polling clients visibly advance even when a selected row is concurrently tombstoned. The 1,000-row transport bound keeps ClickHouse HTTP parameters below the server field-size limit. ClickHouse applies `LIMIT BY` after query ordering, which lets the cursor select the newest version for each key before applying the active-row filter: <https://clickhouse.com/docs/sql-reference/statements/select/limit-by>.
