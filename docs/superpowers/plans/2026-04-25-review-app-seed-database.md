...
Create `seedNutrition(sql, random)` that inserts 90 days of `nutrition_daily`, recent `food_entry` rows with `food_entry_nutrient`, and at least three supplement schedules with versioned `supplement_definition` and `supplement_definition_nutrient` rows.
