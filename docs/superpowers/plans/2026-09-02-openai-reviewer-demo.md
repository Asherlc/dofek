...
**Architecture:** A narrowly scoped TypeScript operational fixture resolves exactly `asherlc+openai-review@asherlc.com` and writes only rows with dedicated synthetic-review provenance through the canonical relational paths. A preflight verifies the production ClickHouse analytics store needed by the MCP health tools. The MCP explorer uses the existing server-side health-series service, but returns the Apps SDK envelope expected by the registered UI resource. Focused route tests cover the public tool contracts; authenticated production calls provide final evidence.
...
- Seed canonical provider-attributed relational records and do not introduce provider-estimated energy data.
...
**Interfaces:**
- Consumes: `DATABASE_URL`, the exact reviewer email, and the configured production analytics health preflight.
- Produces: deterministic Postgres HRV/step samples for 2026-08-18 through 2026-08-31, seven sleep sessions for 2026-08-25 through 2026-08-31, several activities in the range, and connected providers with fixed last-sync timestamps.
...
Resolve one `fitness.user_profile` record by the exact email. Delete or upsert only rows joined to that user and carrying `SYNTHETIC_PROVENANCE`, including the provider-attributed daily-metrics values. Before production validation, verify the existing ClickHouse analytics dependency is healthy; do not write duplicate HRV or step data to ClickHouse. Store synthetic provenance in existing appropriate source/reference fields—do not add schema fields or use provider-specific food columns. Emit record counts only, never row payloads or health values.
