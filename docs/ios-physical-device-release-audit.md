...
| IOS-PHY-08 | Camera barcode scan — Current | Every RC | Open food logging, grant camera access if needed, scan a supported printed barcode, and complete or cancel the flow | Live preview renders; one scan is handled; lookup result or actionable not-found state appears; closing returns to food logging |
