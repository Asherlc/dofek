...
days. It reads only active, canonically grouped `fitness.activity.raw` records
whose `started_at` is in that window. It does not fetch provider data, mutate
raw payloads, update Postgres activity rows, or insert directly into
