...
- **Validate Terraform locally before PRs**: Before creating a PR that touches any Terraform files (`deploy/*.tf`, `deploy/server/cloud-init.yml`, `deploy/docker-compose.deploy.yml`, `deploy/otel-collector-config.yaml`), run `terraform plan` and `terraform apply` yourself using Infisical secrets. Never push Terraform changes that you haven't verified actually work. CI running `terraform apply` is not a substitute for local validation.
