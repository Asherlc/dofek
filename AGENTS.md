...
- **Start integration dependencies first**: Before running any integration tests (including `pnpm test:changed`), follow [`docs/testing.md`](docs/testing.md): generate `.env.local`, start `db`, `clickhouse`, `redis`, and `redpanda`, regenerate the port file, and source it in the shell that launches Vitest. Do not classify `localhost:8123` connection failures or a missing native FIT decoder as product bugs until those prerequisites are satisfied.
