...
export const imuSession = fitness.table(
  "imu_session",
  {
    ...
    accelFreqMode: smallint("accel_freq_mode").notNull().default(1),
    gyroFreqMode: smallint("gyro_freq_mode"),
    rawData: text("raw_data"), // base64-encoded binary
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [
    uniqueIndex("imu_session_user_external_idx").on(table.userId, table.externalId),
