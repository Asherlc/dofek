...
describe("0058 account erasure migration (integration)", () => {
  let admin: Client;
  let database: Client;
  ...
  beforeAll(async () => {
    ...
    const preAccountErasureMigrations = readdirSync(migrationsDirectory)
      .filter((file) => file.endsWith(".sql") && file !== "0058_account_erasure.sql")
      .sort();
    ...
    for (const statement of migrationStatements(
      resolve(migrationsDirectory, "0058_account_erasure.sql"),
    )) {
      await database.query(statement);
    }
  }, 240_000);
