...
interface TrainerRoadLoginResponse {
	// Cookie-based auth — the response sets SharedTrainerRoadAuth cookie
	// We store the cookie value as the access token
	success: boolean;
}
...
export class TrainerRoadClient {
	...
	static async signIn(
		...
		const authCookieEntry = responseCookies.find((c) =>
			c.startsWith("SharedTrainerRoadAuth="),
		);
		if (!authCookieEntry) {
