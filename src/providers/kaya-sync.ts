...
import { ProviderInvalidCredentialsError, ProviderStoredIdentityMissingError } from "./auth-errors.ts";
...
export class KayaSyncProvider implements SyncProvider {
  ...
  async sync(run: SyncRun): Promise<SyncResult> {
    ...
    try {
      ...
      for (const session of sessions) {
        ...
        const row = await upsertProviderActivity(run.db, {
          providerId: this.id, userId, externalId: session.id,
          activityType: resolveProviderActivityType("rock_climbing", "rock_climbing"),
          startedAt: started, endedAt: session.end_time ? new Date(session.end_time) : null,
          name: session.gym ? `Kaya climbing at ${session.gym.name}` : "Kaya climbing",
          sourceName: this.name, raw: session,
        }, {
          activityType: resolveProviderActivityType("rock_climbing", "rock_climbing"),
          startedAt: started, endedAt: session.end_time ? new Date(session.end_time) : null,
          name: session.gym ? `Kaya climbing at ${session.gym.name}` : "Kaya climbing",
        ...
        if (sessionAscents.length) {
          await run.db.insert(climbingEntry).values(sessionAscents.flatMap((ascent) => {
            const boulder = ascent.climb.climb_type.name.toLowerCase().includes("boulder");
            const grade = ascent.climb.grade;
            if (!grade) { errors.push({ message: "Kaya ascent is missing a grade", externalId: ascent.id }); return []; }
            return [{ activityId: row.id, externalId: ascent.id, climbType: boulder ? "boulder" as const : "route" as const,
              gradeSystem: boulder ? "v_scale" as const : "yds" as const, grade: grade.name,
              sent: sentAscentTypes.has(ascent.ascent_type.name.toLowerCase()), attemptCount: ascent.attempts ?? 1,
              lead: boulder ? null : ascent.climb.lead, routeName: ascent.climb.name,
              locationName: ascent.climb.gym?.name ?? session.gym?.name ?? null, sourceName: this.name, raw: ascent }];
          }));
  ...
  #result(startedAt: number, recordsSynced: number, errors: unknown[]): SyncResult {
    return { provider: this.id, recordsSynced, errors: errors.map((error) => ({ message: error instanceof Error ? error.message : String(error) })), duration: Date.now() - startedAt };
  }
}
