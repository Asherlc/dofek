...
describe("TrainerRoadProvider.sync() (integration)", () => {
  ...
  it("requests activities with YYYY-MM-DD date range and the stored username", async () => {
    await saveTokens(ctx.db, "trainerroad", {
      accessToken: "valid-cookie",
      refreshToken: null,
      expiresAt: new Date("2027-01-01T00:00:00Z"),
    ...
    server.use(
      http.get(
        "https://www.trainerroad.com/app/api/calendar/activities/:username",
        ({ params, request }) => {
          const { username } = params;
          capturedUsername = Array.isArray(username) ? username[0] : username;
    ...
    const provider = new TrainerRoadProvider();
    const result = await provider.sync(ctx.db, new Date("2026-02-15T09:30:00.000Z"));

    expect(result.errors).toHaveLength(0);
    // Regex /username:(\S+)/ must capture the full username, not a single char.
    expect(capturedUsername).toBe("testuser");
    // formatDate must produce a YYYY-MM-DD string (no time component).
    expect(capturedStartDate).toBe("2026-02-15");
    expect(capturedStartDate).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    expect(capturedEndDate).toMatch(/^\d{4}-\d{2}-\d{2}$/);
  });

  it("reports a positive, bounded duration on a successful sync", async () => {
    await saveTokens(ctx.db, "trainerroad", {
      accessToken: "valid-cookie",
      refreshToken: null,
      expiresAt: new Date("2027-01-01T00:00:00Z"),
    ...
    server.use(...trainerroadHandlers([fakeActivity({ Id: 8001 })]));
  ...
  it("reports a positive, bounded duration on the early-return error path", async () => {
    await ctx.db.delete(oauthToken).where(eq(oauthToken.providerId, "trainerroad"));
