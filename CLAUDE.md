...
## Development Rules
- **Fix properly, no workarounds**: When encountering an issue, fix the root cause. Don't disable lint rules, skip checks, add ignores, or use workarounds to make problems go away. When in doubt, do the harder thing that actually solves the problem.
...
- **Zod for external data boundaries**: Use Zod schemas to parse and validate data from external sources — API responses, database query results, uploaded files, webhook payloads. Prefer `executeWithSchema()` (from `packages/server/src/lib/typed-sql.ts`) over `db.execute<T>()` generics for raw SQL queries. TypeScript generics only provide compile-time safety; Zod catches runtime shape mismatches.
...
- **No coverage exclusions for convenience**: Never exclude source files from code coverage just to keep thresholds high. If a file has low coverage, write tests for it — don't hide it. The only legitimate coverage exclusions are test files themselves, `__tests__/` directories, and `node_modules/`.
