...
- **Colocated tests**: Test files live next to the source file they test, named `<source>.test.ts` (unit) or `<source>.integration.test.ts` (integration). Do not use `__tests__/` directories. For example, `src/db/tokens.ts` has tests at `src/db/tokens.test.ts` and `src/db/tokens.integration.test.ts`.
