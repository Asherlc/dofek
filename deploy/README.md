...
- **Storage**:
  ...
  - **Redpanda**: Runs internally in the swarm as the hot ingest log for high-volume `metric_stream` events. ClickHouse consumes the topic for analytics, and Redpanda Connect archives it to R2.
  - **PeerDB**: Runs internally in the swarm as the Postgres-to-ClickHouse CDC service for lower-volume raw fitness tables into `postgres_fitness.*`. `metric_stream` is not mirrored by PeerDB; it is written to ClickHouse by the Redpanda ClickHouse sink.
