...
- All production entrypoint modes that run dbt use `--threads 1 --select $DBT_SAFE_MODELS` to avoid concurrent or unsafe ClickHouse model builds on the single-node host. `analytics-worker` runs `sensor_scalar_sample`, `deduped_sensor`, and `resting_heart_rate_sleep_window` every 15 minutes so expensive ClickHouse derived tables are updated outside web/API and BullMQ worker request paths. `analytics.activity_summary` is served from the incremental `analytics.activity_summary_rows` table through a thin view so dashboard workload routes do not recompute activity/sample joins on demand. `activity_summary_rows` is currently excluded from the production dbt run path because the first single-query offline version still OOM-killed ClickHouse; split it into smaller chained incremental models before enabling it. Failed dbt runs sleep before retrying instead of exiting into an immediate Swarm restart loop.
