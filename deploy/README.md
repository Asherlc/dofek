...
- **Secrets**: Managed via **Infisical**. CI exports tagged deploy secrets (`--tags=deploy-env`) into a temporary `.env.prod` file on the runner for `docker stack deploy`. Multiline secrets (for example `APPLE_PRIVATE_KEY`) are injected as dedicated Docker Swarm secrets. The server never stores secrets on disk.
...
3. **Deploy App** (`deploy-app.yml`):
   1. Install Infisical CLI, export deploy-tagged secrets to `$RUNNER_TEMP/.env.prod`, and fetch `APPLE_PRIVATE_KEY` separately to a temporary file.
   ...
   7. Create/reuse a content-addressed Docker secret for `APPLE_PRIVATE_KEY` and export `APPLE_PRIVATE_KEY_SECRET_NAME` for stack interpolation.
