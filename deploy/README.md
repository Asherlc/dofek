...
- All production entrypoint modes that run dbt use `--threads 1 --select $DBT_SAFE_MODELS` to avoid concurrent or unsafe ClickHouse model builds on the single-node host. `analytics-worker` currently runs only `sensor_scalar_sample` every 15 minutes. `deduped_sensor`, `resting_heart_rate_sleep_window`, and `activity_summary_rows` are excluded because the first single-query incremental versions OOM-killed production ClickHouse; split them into smaller chained incremental models before enabling them. `analytics.activity_summary` is served from the incremental `analytics.activity_summary_rows` table through a thin view so dashboard workload routes do not recompute activity/sample joins on demand. Failed dbt runs sleep before retrying instead of exiting into an immediate Swarm restart loop.
