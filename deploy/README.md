...
- ClickHouse has a 3500 MiB container memory limit and a checked-in 3 GiB `max_server_memory_usage` cap so large analytics queries fail inside ClickHouse instead of triggering host-level OOM kills on the single-node host. It also loads checked-in server profile settings from `deploy/clickhouse/users.d/` and server settings from `deploy/clickhouse/config.d/`; the production stack mounts these as Docker Swarm configs so app-managed geospatial `Nullable(Point)` columns, bounded memory, and seven-day system-log TTL retention work without manual server changes. Docker Swarm config contents are immutable, so changing a checked-in config file must also rotate the config key in `deploy/stack.yml` (for example `clickhouse_memory_limits_3g`) instead of reusing the same config name with new contents.
- All production entrypoint modes that run dbt use `--threads 1 --select $DBT_SAFE_MODELS` to avoid concurrent or unsafe ClickHouse model builds on the single-node host. `analytics-worker` currently runs the dbt-native microbatched `sensor_scalar_sample` and `deduped_sensor` models plus the dirty-keyed `activity_vo2max_estimate` model every 15 minutes. `resting_heart_rate_sleep_window` and `activity_summary_rows` are excluded because the first single-query incremental versions OOM-killed production ClickHouse; convert them to bounded dbt-native strategies before enabling them. `analytics.activity_summary` is served from the incremental `analytics.activity_summary_rows` table through a thin view so dashboard workload routes do not recompute activity/sample joins on demand. Failed dbt runs sleep before retrying instead of exiting into an immediate Swarm restart loop. Staging applies `deploy/stack.staging.yml` and keeps `analytics-worker` at zero replicas because the 4 GB staging host cannot run scheduled dbt work alongside ClickHouse and PeerDB without OOM-thrashing.
