...
- **Deterministic Migrations**: CI first applies the dependency-only stack and
  waits for Postgres and ClickHouse, then runs the image's one-shot migrations,
  applies the quiesced application stack, completes cutover and CDC gates, and
  finally restores ClickHouse consumers. Do not run migrations inside `web`
  startup or reorder these phases.
...
`deploy/stack.yml` and `deploy/stack.oracle.yml` in dependency, quiesced, and
full-stack phases around the migration and CDC gates.
